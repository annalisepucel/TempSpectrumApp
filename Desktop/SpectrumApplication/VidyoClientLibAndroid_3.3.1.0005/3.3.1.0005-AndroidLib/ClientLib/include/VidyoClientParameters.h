...
typedef struct VidyoClientInEventShare_
{
	/*!
		...
		If shareType is VIDYO_CLIENT_CONTENTS_SHARE_TYPE_DESKTOP_WINDOW the value
		in field window should be one of the value in field sysDesktopId in
		#VidyoClientRequestGetWindowsAndDesktops. No other value is accepted.
	*/
...
/*! Event structure used to select speaker

	- Used by following in events:
		+ #VIDYO_CLIENT_IN_EVENT_SELECT_SPEAKER
		+ #currently only supported in iOS
*/
...
#define VIDYO_CLIENT_PORTAL_SEARCH_SIZE		10

/*! Room mode

	- Used for the following events:
		+ #VVIDYO_CLIENT_OUT_EVENT_PORTAL_SERVICE / VIDYO_CLIENT_SERVICE_SEARCH
...
/*! Event structure used to send response for generic command to portal.
...
typedef struct VidyoClientRequestGetWindowsAndDesktops_
{
	...
	VidyoUint sysDesktopId[MAX_SHARE_DISPLAY_DEVICE];
